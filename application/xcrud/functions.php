...
function image_thumb($value, $fieldname, $primary, $row, $xcrud) {
    exit("SDfds");
    return '<img src="'.'https://www.travelhope.com/assets/uploads/'.$row['ota_registration.ota_id'].'/thumbs/'.$value.'" />';
}
function invoice_url($value, $fieldname, $primary, $row, $xcrud) {
    if($row["modules.name"] == "hotels")
    {
        $module_name = "hotels";
    }else{
        $module_name = "flights";
    }
    return "<a  class='btn btn-primary  btn-block' target='_blank' href=".base_url("$module_name/invoice/".$value)."><strong>Invoice</strong></a>";
}

function booking_status($postdata, $primary, $xcrud){
    
    if($postdata->get('bookings.status') == 'confirmed'){
        $ci =& get_instance();
        
        // request for send booking confirmation email
        $params['ota_id'] = $ci->session->userdata('clientId');
        $params['supplier'] = $ci->session->userdata('supplier_email');
        $data['data'] = json_decode(server_request($params, $ci->session->userdata('endPoint').'hotels/booking/confirm'));
    }
    
    $postdata->set('bookings.status', $postdata->get('bookings.status'));
}
