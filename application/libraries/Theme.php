...
class Theme
{
    ...
    function __construct()
    {
        ...
        $this->_data = array('theme_url' => $uu . $ptheme . '/', 'themeurl' => 'themes/' . $ptheme . '/', 'api_base_url' => $this->config('api_base_url'), 'clientID' => $this->_ci->session->userdata('clientId'), 'theme_name' => $ptheme, 'include_path' => $this->config('path') . $ptheme . '/', 'supplier_logged_in' => $this->_ci->session->userdata('supplier_email'));

        $this->_data = array_merge($this->_data, $settings_data);
    }
     ...
     */
    function view($view, $data = array(), $return = false, $partial = false)
    {
        $content = $this->partial($view, $data, true);
        if (!$partial) {
            return $this->render($content, $return, $data);
        } else {
            return $content;
        }
    }
     ...
     * @return void or HTML
     */
    function render($content, $return = false, $data = [])
    {
